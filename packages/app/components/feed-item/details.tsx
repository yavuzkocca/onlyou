...
export const NFTDetails = ({ nft, edition, user, detail }: NFTDetailsProps) => {
  const isDark = useIsDarkMode();
  const { shareNFT } = useShareNFT();
  const isCreatorDrop = !!nft.creator_address;
  //console.log(`NFT Details=${nft.token_name}`);
      ...
      <View tw="flex-row justify-between py-2">
        ...
        <View tw="flex-row">
          ...
          <Suspense fallback={<Skeleton width={24} height={24} />}>
            <NFTDropdown nft={nft} tw="" user={user} />
          </Suspense>
        </View>
      </View>
      <ClaimedBy
