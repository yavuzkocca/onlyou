...
interface CommentInputBoxProps {
  ...
  style?: ViewStyle;
  commentInputRef?: any;
  submit: (message: string, parentId?: string | null) => Promise<void>;
}
...
>(function CommentInputBox({ submitting, submit, commentInputRef }, ref) {
  ...
  const { user } = useUser();
  //#endregion
  console.log("NRT" + JSON.stringify(user));
  ...
  const handleOnSubmitComment = useCallback(
    async function handleOnSubmitComment(text: string) {
      const _newComment = async () => {
        try {
          await submit(text, selectedComment?._id);
  ...
  return (
    <>
      ...
      <MessageBox
        ...
        userAvatar={user?.data?.data?.profile?.img_url}
      />
    </>
  );
});
