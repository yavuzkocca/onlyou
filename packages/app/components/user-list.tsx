...
export const UserList = ({
  ...
  const keyExtractor = useCallback((item: MyInfo) => `${item.profile._id}`, []);
...
const FollowingListUser = memo(
  ({ item }: { item: MyInfo } & FollowingListProp) => {
    ...
    console.log(`USERLIST=${JSON.stringify(item)}`);
            ...
            <View tw="mr-2 h-8 w-8 rounded-full bg-gray-200">
              {item?.profile?.img_url && (
                <Image
                  ...
                  alt={item.profile?.username ?? item.profile?.wallet_address}
                  tw="rounded-full"
                  width={32}
                  height={32}
                />
