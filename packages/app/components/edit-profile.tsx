...
import { Upload, CheckFilled } from "@showtime-xyz/universal.icon";
...
export const EditProfile = () => {
  ...
  const { isValid, isTaken, validate } = useValidateUsername();
  ...
  const [index, setIndex] = useState(() =>
    !user?.data?.data?.profile?.username ||
      !user?.data?.data?.profile?.bio ||
      !user?.data?.data?.profile?.img_url
      ? 0
      : userHasIncompleteExternalLinks(user.data.data.profile)
        ? 1
        : 0
  );
  ...
  const defaultValues = useMemo(() => {
    const links: any = {};
    if (socialLinks?.data?.data && user?.data.data.profile.links) {
      socialLinks.data.data.forEach((s) => {
        const foundLink = user.data.data.profile.links.find(
          (l) => l.type_id === s.id
        );
    ...
    return {
      name: user?.data?.data?.profile?.name,
      username: user?.data?.data?.profile?.username,
      bio: user?.data?.data?.profile?.bio,
      links,
      website_url: user?.data?.data?.profile?.website_url,
      default_created_sort_id:
        user?.data?.data?.profile?.default_created_sort_id,
      default_list_id: user?.data?.data?.profile?.default_list_id,
      default_owned_sort_id: user?.data?.data?.profile?.default_owned_sort_id,
      profilePicture: user?.data?.data?.profile?.img_url,
      coverPicture: user?.data?.data?.profile?.cover_url,
    };
  }, [socialLinks?.data?.data, user]);
  ...
  const handleSubmitForm = async (values: typeof defaultValues) => {
    ...
    try {
      ...
      let profileIpfsHash;
      if (cP) {
        coverIpfsHash = await upload({
          data: [values.coverPicture],
          options: {
            uploadWithGatewayUrl: true,
            uploadWithoutDirectory: true,
          },
      ...
      await axios({
        url: "/api/users/editname", // Uygun URL'yi buraya yazın
        method: "POST",
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
        data: { newValues, coverIpfsHash, profileIpfsHash },
      });
      ...
      mutate(`/api/users/myinfo?wallet=${wallet}`);
      matchMutate(
                ...
                <DropFileZone onChange={({ file }) => onChange(file)}>
                  <Pressable
                    onPress={async () => {
                      ...
                      const uri = getLocalFileURI(file); // file.uri
                      console.log(`URI=${uri}`);

                      // onChange(JSON.stringify(file.file));
                      onChange(file);
                      // console.log(`OnChange=${JSON.stringify(file.file)}`);
                      setSelectedImg(uri);
                      // console.log(`SelectedImg=${uri}`);
                      setCurrentCropField("coverPicture");
                      // console.log(`CurrentCropField=${currentCropField}`);
                      setCP(true);
                    }}
                    style={{
