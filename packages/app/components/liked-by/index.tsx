  ...
  return (
    ...
    >
      ...
      <Text tw="text-sm text-gray-900 dark:text-white">
        {data?.slice(0, max).map((like, index) => (
          <Fragment key={`liked-by-user-${like.profile_id}`}>
            <TextLink
              href={`/@${like.username ?? like.wallet_address}?type=created`}
        ...
        {(data?.length ?? 0) > 2 && (
          ...
          >
            ...
            <Text tw="text-sm font-bold text-black dark:text-white">
              {data
                ? `${data.length - 2} ${data.length - 2 === 1 ? "other" : "others"
                }`
                : 0}
            </Text>
          </Text>
