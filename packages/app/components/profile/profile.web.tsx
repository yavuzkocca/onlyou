...
import { Card } from "app/components/card-profile-tabs";
...
const Profile = ({ username }: ProfileScreenProps) => {
  ...
  // console.log(`ProfileDataWEB=${JSON.stringify(profileData.data.profile)}`);
  const profileId = profileData?.data?.profile._id;
  // console.log(`profileId=${profileId}`);
  const { getIsBlocked } = useBlock();
  const isBlocked = getIsBlocked(profileId);
  const { user } = useUser();
  console.log(`PRUSEUSER ${JSON.stringify(user?.data?.data.profile?._id)}`);
  ...
  console.log(`PROFLIUST ${JSON.stringify(list)}`);
          ...
          {chuckItem.map((item, chuckItemIndex) => (
            <Card
              ...
              href={`/list?initialScrollIndex=${itemIndex * numColumns + chuckItemIndex
                }&tabType=${type}&profileId=${profileId}&collectionId=${filter.collectionId
        ...
        <View tw="w-full">
          <MutateProvider mutate={updateItem}>
            <ProfileTabsNFTProvider
              tabType={
                user?.data?.data.profile?._id === profileId ? type : undefined
              }
            >
              <InfiniteScrollList
