...
const SuggestedUsers = () => {
  ...
  console.log(`SUGGDATA ${JSON.stringify(data)}`);
  console.log(`SUGGDATAUSER ${JSON.stringify(user)}`);

  const propertyToFilterByArray1 = "profile_id";
  const propertyToFilterByArray2 = "_id";

  const filteredArray1 = data.filter(
    (obj1) =>
      !user.some(
        (obj2) =>
          obj2[propertyToFilterByArray2] === obj1[propertyToFilterByArray1]
      )
  );
    ...
    <>
      ...
      <View tw="dark:shadow-dark shadow-light mt-8 rounded-2xl bg-white dark:bg-black">
        ...
        {data?.data?.map((user, index) => {
          // const isFollowingUser = isFollowing(user?.profile_id);
          // console.log(`isFollowingUserSug ${isFollowingUser}`);
