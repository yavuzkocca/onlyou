...
export const DropEvent = () => {
  ...
  if (state.status === "success") {
    const claimPath = `/nft/${[process.env.NEXT_PUBLIC_CHAIN_ID]}/${state.edition?.contract_address
      }/0`;
  ...
  const handleFileChange2 = (fileObj: FilePickerResolveValue) => {
    ...
    if (
      ...
      setError("file2", { type: "custom", message: "File type not supported" });
      setValue("file2", undefined);
    } else {
      clearErrors("file2");
      setValue("file2", file);
  ...
  return (
    <BottomSheetModalProvider>
      ...
      <BottomSheetScrollView ref={scrollViewRef} style={{ padding: 16 }}>
        <View>
          <View tw="mb-3 flex-row justify-around">
            <Controller
              control={control}
              name="file"
              render={({ field: { value } }) => {
            ...
            <Controller
              ...
              render={({ field: { value } }) => {
                return (
                  <DropFileZone onChange={handleFileChange2}>
                    <View tw="z-1">
                      <Pressable
                        onPress={async () => {
                          const file = await pickFile({
              ...
              render={({ field: { onChange, onBlur, value } }) => {
                return (
                  <Fieldset
                    ...
                    label="Price"
                    onBlur={onBlur}
                    helperText="Enter Price per NFT"
          ...
          <View tw="mb-4 flex-row">
            <Text tw="pb-2 text-sm text-gray-600 dark:text-gray-200">
              ...
              {formatAddressShort(wallet_address)}
            </Text>
          </View>
