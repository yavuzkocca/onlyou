...
export function MessageRow({
  ...
  const createdAtText = useMemo(() => {
    ...
    if (differenceInSeconds(currentTime, createdAt) < 10) {
      return "now";
    }
    const createdAtx = moment.unix(createdAt);
    return moment(createdAtx).format("MM.DD.YY, HH:MM");
  }, [createdAt]);
  ...
  return (
    <View tw="flex flex-row bg-white py-4 dark:bg-black">
      ...
      <View tw="items-center">
        ...
        <Link href={`/@${username || address}?type=created`}>
          <Button
            variant="secondary"
            size="small"
            tw="h-[24px] w-[24px]"
