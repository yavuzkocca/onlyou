...
export const NotificationItem = ({
  ...
  console.log(`NOTFCATUSR= ${JSON.stringify(myProfile)}`);
  ...
  const avatarLink =
    ...
    (actor?.wallet_address
      ...
      : `${myProfile?.user?.data.data.profile.username ||
      myProfile?.user?.data.data.profile.wallet_addresses
      }`);
...
const NotificationDescription = ({
  ...
  const now = fromUnixTime(notification.to_timestamp);
  ...
  return (
    <View tw="flex-1">
      ...
      >
        ...
        {/* {Boolean(getFormatDistanceStrictToWeek(distance)) && (
          <Text tw="text-13">{` · ${getFormatDistanceStrictToWeek(
            distance
          )}`}</Text>
        )} */}
        <Text tw="text-13">{`  ${distance} ago`}</Text>
      </Text>
    </View>
  );
};
...
export const useNotificationIcon = (type_name: string) => {
  switch (type_name) {
    ...
    case "COMMENT_MENTION": //OK
      return (
        <MessageFilled width={20} height={20} color={colors.indigo[500]} />
      );
    case "LIKE_ON_COMMENT": //OK
      return <HeartFilled width={20} height={20} color={colors.rose[500]} />;
    case "NFT_SALE":
      return <MarketFilled width={20} height={20} color={colors.amber[500]} />;
    case "NEW_CREATOR_AIRDROP_FROM_FOLLOWING": //OK
    ...
    case "CREATED_EDITION_SOLD_OUT": //OK
