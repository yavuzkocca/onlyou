...
const CardLargeScreen = ({
  ...
  const { showPrivImage } = useShareNFT();

  const hasPrivImage = nft?.has_priv_image;

  const edition = detailData?.data?.nft_id;
  // console.log(`Edition=${edition}`);
  console.log(`NFTCard=${JSON.stringify(detailData)}`);
  console.log(detailData?.data.multiple_owners_list);
  return (
        ...
        <View tw="pb-4" shouldRasterizeIOS={true}>
          ...
          <RouteComponent href={href!} onPress={handleOnPress}>
            <Media
              ...
              alt={nft.token_name}
            />
            {numColumns === 1 && nft?.content_type?.includes("video") ? (
          ...
          <View tw="flex-row justify-between px-4 py-2">
            <Social nft={nft} />
            {showClaimButton && edition ? (
              <View tw="flex-row">
                <ClaimButton edition={edition} nft={nft} user={user} />
            ...
            {hasPrivImage && (
              <Button
                ...
                tw="rounded-full bg-gray-100 p-1 dark:bg-gray-900"
                onPress={async () => {
                  const url = await showPrivImage(nft);
          ...
          <ClaimedBy
            claimersList={detailData?.data.multiple_owners_list}
            nft={nft}
            tw="px-4"
          />
        </View>
