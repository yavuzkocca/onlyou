...
export function AuthProvider({
  ...
  const login = useCallback(
    async function login(endpoint: string, data: object): Promise<MyInfo> {
      // const respons = await fetchOnAppForeground({
      ...
      const wallet = response?.wallet.toLowerCase();
      localStorage.setItem("wallet", wallet);

      //console.log(res);
      const res = await axios({
        url: `${MY_INFO_ENDPOINT}?wallet=${wallet}`,
        method: "GET",
      });
      ...
      if (validResponse && res) {
        ...
        logoutStorage.deleteLogout();
        console.log(`COMPRES ${JSON.stringify(res.data.data.profile)}`);
        const isIncomplete = isProfileIncomplete(res?.data.data.profile);
        ...
        if (isIncomplete) {
          ...
          setTimeout(() => {
            console.log("COMPRES" + isIncomplete);
            router.push(
              Platform.select({
                native: "/profile/edit",
                web: "/profile/edit",
  ...
  const logout = useCallback(
    async function logout() {
      ...
      // await axios({
      //   url: "api/auth/logout_wallet",
      //   method: "GET",
      // });
      // console.log("LOGOUT ÇALIŞTIIIIIIIIIIIIIIIIIIII");
